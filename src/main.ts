...
function extractFilePathsFromText(text: string, cwd: string): string[] {
  const paths: string[] = [];
  // Match absolute paths (macOS/Linux/Windows) and tilde paths with a file extension
  const regex = /(?:\/(?:Users|home|tmp|var|etc)\/[^\s`'"()\[\]{}|<>]+\.\w+|~\/[^\s`'"()\[\]{}|<>]+\.\w+|[A-Za-z]:\\(?:[^\\\s`'"()\[\]{}|<>]+\\)*[^\\\s`'"()\[\]{}|<>]+\.\w+)/g;
...
async function runDaemon(): Promise<void> {
  ...
  function shutdown(): void {
    ...
    monitor.stop();
    // On Windows, process events like SIGINT/SIGTERM are not available in the same way.
    // process.exit() is the cleanest option.
    process.exit(0);
  }

  // process.on('SIGINT') and 'SIGTERM' may not fire on Windows when run as a background process.
  // We register them anyway for cases where the daemon runs in a visible console window.
